...
const QuestionerDetail: React.FC<IQuestionerDetailProps> = () => {
    ...
    const [showStickySaveBtn, setShowStickySaveBtn] = useState(false);
    ...
    function cleanUpAnswersForSave() {
        ...
        if (state.showOnlyUnanswered) {
            currentAnswerItem.ANSWERS = currentAnswerItem.ANSWERS.filter((item: IAnswer) => !item.answer);
            currentAnswerItem.ANSWERS = [...state.answerItem.ANSWERS, ...state.answerItemWithUnanswered.ANSWERS].reduce((accumulator, current) => {
                ...
                index === -1 ? accumulator.push(current) : (accumulator[index] = current);
                return accumulator;
            }, []);
            let currentAnswerItemWithUnanswered = JSON.parse(JSON.stringify(state.answerItemWithUnanswered));
            currentAnswerItemWithUnanswered.ANSWERS = currentAnswerItemWithUnanswered.ANSWERS.filter((item: IAnswer) => !item.answer);
    ...
    function answerChangedActionPerformed(questionNr: string, answer: string) {
        ...
        setState((prevState) => {
            prevState[currentAnswerItem].ANSWERS.forEach((itm: IAnswer) => {
                if (itm.number === questionNr) {
                    ...
                    if (itm.childQuestions && itm.childQuestions.toString() !== "") {
                        prefillChildAnswers(prevState[currentAnswerItem].ANSWERS, itm.childQuestions, answer);
                    }
                    return (itm.answer = answer);
                }
            });
            return { ...state, [currentAnswerItem]: prevState[currentAnswerItem] };
        });
    }
    ...
    function getOnlyUnanswered() {
        if (!state.showOnlyUnanswered) {
            ...
            setState({ ...state, showOnlyUnanswered: !state.showOnlyUnanswered, showOnlyUnansweredConfirmation: false });
        }
        void saveData(false);
    }
    ...
    return (
        <>
            ...
            <ConfirmationModal
                ...
                title={state.offlineMode === true ? "Wieder in online Modus wechseln?" : "Offline-Modus aktivieren?"}
                message={
            ...
            <div
                style={{
                    ...
                    zIndex: 30,
                    opacity: showStickySaveBtn ? 1 : 0,
                    visibility: showStickySaveBtn ? "visible" : "hidden"
                }}>
                {console.log(showStickySaveBtn)}
                <ActionButton
                    id="stickySaveBtn"
